...
async function getMasterStatusReport(req, res) {
  try {
    // Extract filter parameters from query string
    const filters = {
      date_from: req.query.date_from || null,
    ...
    console.log(`📊 MASTER STATUS REPORT REQUEST: User ${userContext.userId} (${userContext.userRole}) requesting report with filters:`, filters);

    // Generate the master status report
    const reportResult = await generateMasterStatusReport(filters, userContext);
    ...
    console.error("Error in getMasterStatusReport controller:", error);
    return res.status(500).json({
      success: false,
