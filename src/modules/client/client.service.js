...
async function createClient(clientData, cellAssignmentData = {}) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const userData of clientUsers) {
        ...
        const clientUser = await tx.clientUser.create({
          data: {
            ...
            is_active: true,
            created_by: cellAssignmentData.assigned_by,
            notes: `User created during client creation for ${client.client_type === "JURIDICO" ? client.company_name : `${client.first_names} ${client.last_name}`}`
          }
        });
      ...
      // Get primary user credentials
      const primaryUser = createdClientUsers.find(cu => cu.credentials.is_primary || createdClientUsers[0]);
      const primaryCredentials = primaryUser ? primaryUser.credentials : null;
      ...
      return {
        ...
        _autoCredentials: primaryCredentials ? {
        ...
        _allUserCredentials: createdClientUsers.map(cu => ({
          username: cu.credentials.username,
          password: cu.credentials.password,
          is_primary: cu.credentials.is_primary
        }))
      };
    }, {
    ...
    try {
      ...
      // Get primary user credentials for email
      const primaryCredentials = result._autoCredentials;
      ...
      if (primaryCredentials) {
        ...
        console.warn(`⚠️ No primary credentials found for client ${result.client_id} - skipping onboarding email`);
        result._emailNotification = {
          sent: false,
          error: "No primary credentials available",
          note: "Email notification skipped - no credentials to send"
        };
      }
    } catch (emailError) {
